...
//            },
            success: function(data){
                if(data.code == 200){
                    $("#uploadState").text("成功:"+data.data.path)
                }else{
                    $("#uploadState").text("失败:"+data.err)
                }
            },
...
function uploadFiles(id){
    var options = {
        ...
        add: function (e, data) {
            ...
            var acceptFileTypes = /(\.|\/)(pdf|pptx?|docx?|xlsx?|xps|jpe?g|png|bmp)$/i;
            if(!checkFileType(filename,acceptFileTypes)){
                return alert("只能上传以下格式的文件：pdf，ppt，pptx，doc，docx，xls，xlsx，xps；\n图片：gif，jpg，jpeg，png，bmp；");//\n音视频：mp3，avi，wmv，mpg，mp4；");
                //return alert("只能上传以下格式的文件：pdf，ppt，pptx，doc，docx，xls，xlsx，xps。")
            }
            $("#uploadDiv").attr("disabled","disabled");
            $("#fileProgress").modal('show');
